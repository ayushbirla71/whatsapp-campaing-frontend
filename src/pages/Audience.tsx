...
const Audience: React.FC = () => {
  ...
  const [showAllAudience, setShowAllAudience] = useState<boolean>(false);
  ...
  useEffect(() => {
    if (showAllAudience) {
      // Fetch global audience
      const fetchGlobalAudience = async () => {
        setLoading(true);
        setError(null);
        try {
          const res = await apiService.getGlobalAudience(
            page,
            limit,
            search,
            countryCode,
          ...
          const mapped = rawList.map((item: any) => ({
            _id:
              item._id ||
              item.id ||
              `${item.msisdn || item.phone_number || ""}-${item.name || ""}`,
          ...
          setError(e.message || "Failed to load global audience");
        } finally {
          setLoading(false);
        }
      };
      ...
      // Existing organization-specific fetch logic
      const fetchAudience = async () => {
        setLoading(true);
        setError(null);
        try {
          const res = await apiService.getOrganizationAudience(
            selectedOrgId,
            page,
            limit
          );
  ...
  }, [selectedOrgId, showAllAudience, page, limit, search, countryCode]);
  ...
  const handleCreateMasterSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // refresh list
      const res = await apiService.getOrganizationAudience(
        selectedOrgId || createMasterOrgId,
        page,
        limit
      );
      const responsePayload: any = res?.data || {};
      const rawList: any[] =
        responsePayload.audience ||
        responsePayload.members ||
        responsePayload.data?.audience ||
        responsePayload.data?.members ||
        [];
      const mapped = rawList.map((item: any) => ({
        _id:
          item._id ||
        ...
        attributes: item.attributes || item.meta || {},
        organization_id:
          item.organization_id || selectedOrgId || createMasterOrgId,
        organization: item.organization,
        created_by: item.created_by || "",
        created_by_user: item.created_by_user,
        created_at:
      ...
      setAudience(mapped);
      const pg =
        responsePayload.pagination || responsePayload.data?.pagination || {};
      setTotalPages(pg.totalPages || 1);
    } catch (e: any) {
  ...
  const handleBulkSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiService.bulkCreateMasterAudience(bulkOrgId, {
        records: csvRecords,
      });
      ...
      // refresh list
      const res = await apiService.getOrganizationAudience(
        selectedOrgId || bulkOrgId,
        page,
        limit
      );
      const responsePayload2: any = res?.data || {};
      const rawList: any[] =
        responsePayload2.audience ||
        responsePayload2.members ||
        responsePayload2.data?.audience ||
        responsePayload2.data?.members ||
        [];
      const mapped = rawList.map((item: any) => ({
        _id:
          item._id ||
      ...
      setAudience(mapped);
      const pg =
        responsePayload2.pagination || responsePayload2.data?.pagination || {};
      setTotalPages(pg.totalPages || 1);
    } catch (e: any) {
  ...
  const handleCsvSelected = (file: File) => {
    ...
    reader.onload = () => {
      try {
        ...
        if (!requiredIdx) {
          setError(
            "CSV must include at least msisdn and name (or first_name,last_name) headers."
          );
          setCsvFileName("");
          setCsvRecords([]);
          return;
          ...
          .map((line) => {
            ...
            const first_name =
              nameIdx !== -1
                ...
                : "";
            const last_name =
              nameIdx !== -1 ? "" : lastNameIdx !== -1 ? cols[lastNameIdx] : "";
            const msisdn = msisdnIdx !== -1 ? cols[msisdnIdx] : "";
            const attributes: any = {};
            if (accountIdx !== -1 && cols[accountIdx])
              attributes.account_no = cols[accountIdx];
            if (cityIdx !== -1 && cols[cityIdx])
              attributes.city = cols[cityIdx];
            if (ageIdx !== -1 && cols[ageIdx]) attributes.age = cols[ageIdx];
            if (subIdx !== -1 && cols[subIdx])
              attributes.subscription = cols[subIdx];
            return {
        ...
        if (!recs.length) {
          setError(
            "No valid rows found in CSV. Ensure msisdn and name are present."
          );
          setCsvFileName("");
          setCsvRecords([]);
          return;
  ...
  return (
    <div className="p-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              Manage master audience records and bulk operations
            </p>
          </div>
      ...
      <div className="mb-4 flex gap-4 items-end">
        <div className="flex-1">
          ...
          <select
            ...
            className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-whatsapp-500 focus:border-whatsapp-500"
          >
      ...
      {/* Search and filters for global audience */}
      {showAllAudience && (
        <div className="mb-4 flex gap-4">
          ...
          <div>
            <input
              ...
              className="px-3 py-2 border border-gray-300 rounded-md focus:ring-whatsapp-500 focus:border-whatsapp-500"
            />
          </div>
          <button
            onClick={() => setPage(1)}
            className="px-4 py-2 bg-whatsapp-500 text-white rounded-lg hover:bg-whatsapp-600"
          >
            Search
          </button>
        </div>
                ...
                <div>
                  ...
                  {csvFileName ? (
                    <p className="mt-2 text-xs text-gray-600">
                      Selected: {csvFileName} ({csvRecords.length} rows)
                    </p>
                  ) : (
                    <p className="mt-2 text-xs text-gray-500">
                      Expected headers: name, msisdn, account_no, city, age,
                      subscription
                    </p>
                  )}
                </div>
      ...
      <div className="mt-6 bg-white rounded-lg shadow">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          {error && (
            <div className="mt-4 rounded bg-red-50 text-red-700 p-3 text-sm">
              {error}
            </div>
          )}
        </div>
