...
import { Template, CreateTemplateRequest, UpdateTemplateRequest, TemplateListResponse } from '../types/template';
import { Campaign, CreateCampaignRequest, UpdateCampaignRequest, CampaignListResponse, CampaignStatistics } from '../types/campaign';
import { Audience, CreateAudienceRequest, BulkCreateAudienceRequest, UpdateAudienceRequest, AudienceListResponse, CampaignAudience, CampaignAudienceListResponse } from '../types/audience';
...
class ApiService {
  ...
  async register(userData: RegisterRequest): Promise<ApiResponse> {
    const response: AxiosResponse<ApiResponse> = await this.api.post('/api/users', userData);
    return response.data;
  }
  ...
  async validateToken(): Promise<ApiResponse> {
    const response: AxiosResponse<ApiResponse> = await this.api.post('/api/auth/validate');
    return response.data;
  }

  // Organization endpoints
  async getOrganizations(page = 1, limit = 10): Promise<ApiResponse<{ organizations: Organization[], pagination: any }>> {
    const response: AxiosResponse<ApiResponse> = await this.api.get(`/api/organizations?page=${page}&limit=${limit}`);
    return response.data;
  }

  async getOrganization(id: string): Promise<ApiResponse<Organization>> {
    const response: AxiosResponse<ApiResponse> = await this.api.get(`/api/organizations/${id}`);
    return response.data;
  }

  async createOrganization(data: CreateOrganizationRequest): Promise<ApiResponse<Organization>> {
    const response: AxiosResponse<ApiResponse> = await this.api.post('/api/organizations', data);
    return response.data;
  }

  async updateOrganization(id: string, data: UpdateOrganizationRequest): Promise<ApiResponse<Organization>> {
    const response: AxiosResponse<ApiResponse> = await this.api.put(`/api/organizations/${id}`, data);
    return response.data;
  }

  async deleteOrganization(id: string): Promise<ApiResponse> {
    const response: AxiosResponse<ApiResponse> = await this.api.delete(`/api/organizations/${id}`);
    return response.data;
  }

  async getOrganizationUsers(id: string): Promise<ApiResponse> {
    const response: AxiosResponse<ApiResponse> = await this.api.get(`/api/organizations/${id}/users`);
    return response.data;
  }

  async updateWhatsAppConfig(id: string, config: WhatsAppConfig): Promise<ApiResponse> {
    const response: AxiosResponse<ApiResponse> = await this.api.put(`/organizations/${id}/whatsapp-config`, config);
    return response.data;
  }

  async getWhatsAppConfig(id: string): Promise<ApiResponse<WhatsAppConfig>> {
    const response: AxiosResponse<ApiResponse> = await this.api.get(`/organizations/${id}/whatsapp-config`);
    return response.data;
  }

  // User endpoints
  async getUsers(page = 1, limit = 10, organizationId?: string): Promise<ApiResponse<UserListResponse>> {
  ...
  async getUser(id: string): Promise<ApiResponse<User>> {
    const response: AxiosResponse<ApiResponse> = await this.api.get(`/api/users/${id}`);
    return response.data;
  }

  async createUser(data: CreateUserRequest): Promise<ApiResponse<User>> {
    const response: AxiosResponse<ApiResponse> = await this.api.post('/api/users', data);
    return response.data;
  }

  async updateUser(id: string, data: UpdateUserRequest): Promise<ApiResponse<User>> {
    const response: AxiosResponse<ApiResponse> = await this.api.put(`/api/users/${id}`, data);
    return response.data;
  }

  async deleteUser(id: string): Promise<ApiResponse> {
    const response: AxiosResponse<ApiResponse> = await this.api.delete(`/api/users/${id}`);
    return response.data;
  }

  // Template endpoints (compatible + backend-specific)
  // 1) Backend-specific: get organization templates
  async getOrganizationTemplates(organizationId: string, page = 1, limit = 10): Promise<ApiResponse<TemplateListResponse>> {
    const params = new URLSearchParams({ page: page.toString(), limit: limit.toString() });
    const response: AxiosResponse<ApiResponse> = await this.api.get(`/api/templates/organization/${organizationId}?${params}`);
    return response.data;
  }

  // 2) Backend-specific: create template for an organization
  async createTemplateForOrganization(organizationId: string, data: CreateTemplateRequest): Promise<ApiResponse<Template>> {
    const response: AxiosResponse<ApiResponse> = await this.api.post(`/api/templates/organization/${organizationId}`, data);
    return response.data;
  }

  // 3) Backend-specific: get pending approval templates (admin review queue)
  async getPendingApprovalTemplates(page = 1, limit = 10): Promise<ApiResponse<TemplateListResponse>> {
    const params = new URLSearchParams({ page: page.toString(), limit: limit.toString() });
    const response: AxiosResponse<ApiResponse> = await this.api.get(`/api/templates/pending-approval?${params}`);
    return response.data;
  }

  // 4) Backend-specific: sync from WhatsApp API for an organization
  async syncTemplatesFromWhatsApp(organizationId: string): Promise<ApiResponse> {
    const response: AxiosResponse<ApiResponse> = await this.api.post(`/api/templates/organization/${organizationId}/sync-whatsapp`);
    return response.data;
  }

  // Legacy-compatible generic list (kept for compatibility where used)
  async getTemplates(page = 1, limit = 10, organizationId?: string, status?: string): Promise<ApiResponse<TemplateListResponse>> {
    const params = new URLSearchParams({ page: page.toString(), limit: limit.toString() });
    if (organizationId) params.append('organization_id', organizationId);
    if (status) params.append('status', status);
    const response: AxiosResponse<ApiResponse> = await this.api.get(`/api/templates?${params}`);
    return response.data;
  }

  async getTemplate(id: string): Promise<ApiResponse<Template>> {
    const response: AxiosResponse<ApiResponse> = await this.api.get(`/api/templates/${id}`);
    return response.data;
  }
  ...
  async createTemplate(data: CreateTemplateRequest): Promise<ApiResponse<Template>> {
    ...
    const response: AxiosResponse<ApiResponse> = await this.api.post('/api/templates', data);
    return response.data;
  }

  async updateTemplate(id: string, data: UpdateTemplateRequest): Promise<ApiResponse<Template>> {
    const response: AxiosResponse<ApiResponse> = await this.api.put(`/api/templates/${id}`, data);
    return response.data;
  }

  async deleteTemplate(id: string): Promise<ApiResponse> {
    const response: AxiosResponse<ApiResponse> = await this.api.delete(`/api/templates/${id}`);
    return response.data;
  }

  async submitTemplate(id: string): Promise<ApiResponse> {
    // Backend route: /api/templates/:id/submit-approval
    const response: AxiosResponse<ApiResponse> = await this.api.post(`/api/templates/${id}/submit-approval`);
    return response.data;
  }

  async approveTemplate(id: string): Promise<ApiResponse> {
    const response: AxiosResponse<ApiResponse> = await this.api.post(`/api/templates/${id}/approve`);
    return response.data;
  }

  async rejectTemplate(id: string, reason: string): Promise<ApiResponse> {
    const response: AxiosResponse<ApiResponse> = await this.api.post(`/api/templates/${id}/reject`, { reason });
    return response.data;
  }
  ...
  async getCampaign(id: string): Promise<ApiResponse<Campaign>> {
    const response: AxiosResponse<ApiResponse> = await this.api.get(`/api/campaigns/${id}`);
    return response.data;
  }

  async createCampaignForOrganization(organizationId: string, data: CreateCampaignRequest): Promise<ApiResponse<Campaign>> {
    const response: AxiosResponse<ApiResponse> = await this.api.post(`/api/campaigns/organization/${organizationId}`, data);
    return response.data;
  }

  async createCampaign(organizationId: string, data: CreateCampaignRequest): Promise<ApiResponse<Campaign>> {
    if (!organizationId) {
  ...
  async deleteCampaign(id: string): Promise<ApiResponse> {
    const response: AxiosResponse<ApiResponse> = await this.api.delete(`/api/campaigns/${id}`);
    return response.data;
  }
  ...
  async approveCampaign(id: string): Promise<ApiResponse> {
    const response: AxiosResponse<ApiResponse> = await this.api.post(`/api/campaigns/${id}/approve`);
    return response.data;
  }

  async rejectCampaign(id: string, reason: string): Promise<ApiResponse> {
    const response: AxiosResponse<ApiResponse> = await this.api.post(`/api/campaigns/${id}/reject`, { reason });
    return response.data;
  }

  // Pending Campaign Approvals
  async getPendingCampaignApprovals(page = 1, limit = 10): Promise<ApiResponse<CampaignListResponse>> {
    const params = new URLSearchParams({ page: page.toString(), limit: limit.toString() });
    const response: AxiosResponse<ApiResponse> = await this.api.get(`/api/campaigns/pending-approval?${params}`);
    return response.data;
  }

  async startCampaign(id: string): Promise<ApiResponse> {
    const response: AxiosResponse<ApiResponse> = await this.api.post(`/api/campaigns/${id}/start`);
    return response.data;
  }

  async pauseCampaign(id: string): Promise<ApiResponse> {
    const response: AxiosResponse<ApiResponse> = await this.api.post(`/api/campaigns/${id}/pause`);
    return response.data;
  }

  async cancelCampaign(id: string): Promise<ApiResponse> {
    const response: AxiosResponse<ApiResponse> = await this.api.post(`/api/campaigns/${id}/cancel`);
    return response.data;
  }
  ...
  async getPendingAdminApprovalTemplates(page = 1, limit = 10): Promise<ApiResponse<TemplateListResponse>> {
    const params = new URLSearchParams({ page: page.toString(), limit: limit.toString() });
    const response: AxiosResponse<ApiResponse> = await this.api.get(`/api/templates/pending-admin-approval?${params}`);
    return response.data;
  }

  async approveAdminTemplates(templateId: string, body?: any): Promise<ApiResponse> {
    const response: AxiosResponse<ApiResponse> = await this.api.post(`/api/templates/${templateId}/admin-approve`, body ?? {});
    return response.data;
  }

  async rejectAdminTemplates(templateId: string, body?: any): Promise<ApiResponse> {
    const payload = typeof body === 'string' ? { rejection_reason: body } : (body ?? {});
    const response: AxiosResponse<ApiResponse> = await this.api.post(`/api/templates/${templateId}/admin-reject`, payload);
    return response.data;
  }
  ...
  async removeAudienceFromCampaign(campaignId: string, audienceId: string): Promise<ApiResponse> {
    const response: AxiosResponse<ApiResponse> = await this.api.delete(`/api/campaigns/${campaignId}/audience/${audienceId}`);
    return response.data;
  }
  ...
  async getAssetFiles(templateId: string): Promise<ApiResponse> {
    const response: AxiosResponse<ApiResponse> = await this.api.get(`/api/asset-files/template/${templateId}?include_inactive=false`);
    return response.data;
  }
